...
const convertToEsm = require("./toEsm.mjs")
const { green, red, yellow, blue, cyan } = require("./colors.mjs")
...
async function start() {
  ...
  const globPattern = `${rootPath}/@(${PACKAGES_PATHS.join(
    "|"
  )})/**/package.json`
  ...
  for (let key in counts) {
    ...
    importMap.imports[key] = Object.keys(paths).reduce(
      (maxPath, currentPath) => {
